...
@click.command("polyasm")
@click.argument('hexstring')
@click.option("-p", "--pwn", default=False, flag_value="pwn", help="Use pwntools to assemble")
@click.option("-r", "--rasm2", default=False, flag_value="rasm2", help="Use rasm2 to assemble")
@click.option("-c", "--capstone", default=False, flag_value="capstone", help="Use capstone/keystone to assemble")
@click.option("-A", "--all-backends", default=False, flag_value="all-backends", help="Use all backends to assemble")
@click.option("-a", "--arch", default="amd64", help="Which architectures to assemble for")
def polyasm(hexstring, pwn, rasm2, capstone, all_backends, arch):
    if all_backends:
        pwn = True
        rasm2 = True
        capstone = True
    if pwn:
        print("--> Pwntools Assembly:")
        arch_list = arch.split(",")
        for a in arch_list:
            print(f"arch: {a}")
            pwn_arch = arch_dict[a]['pwntools']
            if pwn_arch:
                output = subprocess.check_output(f"pwn asm -c {pwn_arch} '{hexstring}'", shell=True).hex()
                print(output + "\n")
            else:
               print(f"{a} not supported by pwntools")
    if rasm2:
        print("--> rasm2 Assembly:")
        arch_list = arch.split(",")
        for a in arch_list:
            print(f"arch: {a}")
            ...
            if rasm2_arch:
                output = subprocess.check_output(f"rasm2 -a {rasm2_arch} -b {rasm2_bits} '{hexstring}'", shell=True).decode().strip()
                print(output + "\n")
            else:
               print(f"{a} not supported by rasm2")
    if capstone:
        print("--> Keystone Assembly:")
        arch_list = arch.split(",")
        for a in arch_list:
            print(f"arch: {a}")
            ...
            if capstone_arch:
                try:
                    output = subprocess.check_output(f"kstool {capstone_arch} '{hexstring}'", shell=True).decode().strip()
                    print(output + "\n")
                except subprocess.CalledProcessError:
                    print(f"error: kstool failed to disassemble {hexstring}")
            else:
               print(f"{a} not supported by keystone")
# Common
# arm
# bpf
# hexagon
